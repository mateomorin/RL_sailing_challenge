...
def train(
    ...
    obs_list = []
    for env in envs:
        o, _ = env.reset(options={"random_start": True, "random_velocity": True})
    ...
    raw_obs = [env.reset(options={"random_start": True})[0] for env in envs]
        ...
        for step in range(rollout_steps):
            ...
            for i, env in enumerate(envs):
                ...
                if done:
                    ...
                    # Reset env
                    new_o, _ = env.reset(options={"random_start": True, "random_velocity": True})
                    raw_obs[i] = new_o
                    ep_rewards[i]    = 0.0
                    ep_lengths[i]    = 0
