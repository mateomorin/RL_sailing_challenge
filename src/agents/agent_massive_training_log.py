...
def train(
    ...
    logger.info(f"[PPO] Device: {device}")
    ...
    logger.info(f"[PPO] Début entraînement : {total_steps:,} steps, {n_updates} updates")
    t_start = time.time()
    ...
    logger.info(f"  Durée totale : {(time.time() - t_start)/60:.1f} min")
    logger.info(f"  Taux de succès final : {np.mean(list(recent_success))*100:.1f}%")
    logger.info(f"  Taux de collision final : {np.mean(list(recent_collision))*100:.1f}%")
...
class MyAgent(BaseAgent):
    ...
    def save(self, path: str) -> None:
        ...
        if self._net is not None:
            ...
            logger.info(f"[MyAgent] Poids sauvegardés → {path}")
    ...
    def load(self, path: str) -> None:
        ...
        try:
            ...
            logger.info(f"[MyAgent] Poids chargés depuis {path}")
        except Exception as e:
            logger.warn(f"[MyAgent] Impossible de charger {path}: {e}")
            self._net = None
...
def evaluate(
    ...
    logger.info("═" * 60)
    logger.info(f"  Épisodes      : {n_episodes}")
    logger.info(f"  Taux succès   : {total_success/n_episodes*100:.1f}%")
    logger.info(f"  Taux collision: {total_collision/n_episodes*100:.1f}%")
    logger.info(f"  Score moyen   : {np.mean(all_rewards):.3f}")
    if any(results[sc]['lengths'] for sc in scenarios):
        all_lengths = [l for sc in scenarios for l in results[sc]['lengths']]
        logger.info(f"  Steps moyens (succès) : {np.mean(all_lengths):.1f}")
    logger.info()
    ...
    for sc in scenarios:
        ...
        if n == 0:
            continue
        logger.info(f"  [{sc}]")
        logger.info(f"    Succès   : {r['success']}/{n} ({r['success']/n*100:.0f}%)")
        logger.info(f"    Collision: {r['collision']}/{n}")
        logger.info(f"    Score moy: {np.mean(r['rewards']):.3f}")
        if r['lengths']:
            logger.info(f"    Steps(succ): {np.mean(r['lengths']):.1f}")
        logger.info()
...
if __name__ == "__main__":
    ...
    if args.train:
        logger.info("=" * 60)
        logger.info("ENTRAÎNEMENT PPO — Sailing Challenge")
        logger.info("=" * 60)
        metrics = train(
    ...
    if args.eval:
        logger.info("=" * 60)
        logger.info("ÉVALUATION — Sailing Challenge")
        logger.info("=" * 60)
